		...
		this.each(function () {
			...
			function moveHover() {
				if (!$(this).parent().parent().parent().find('ul li a.clicked').hasClass('clicked')) {
					$link = $(this);
					$(this).parent().parent().parent().find('.hoverline').dequeue().animate({
						'left': $link.parent().position().left,
						'width': $link.parent().width()
					}, 300, 'linear');
					$(this).parent().parent().parent().find('.hoverline').dequeue();
				}
			}

			function clickedItem() {
				if (!$(this).hasClass('currElement')) {
			...
			function leaveHover() {
				if (!$(this).parent().parent().parent().find('ul li a.clicked').hasClass('clicked')) {
					...
					console.log('Move hover: ' + $link.parent().position().left + ' | ' + $link.parent().width());
				}
			}
...
// 			});
			...
			$this.find('ul li a').bind('mouseover', moveHover);
				...
				if (et <= 105 && et + eh >= 100) {
					if (yearsCont[key] == null || yearsCont[key] == false) {
						if ($('#tlm' + key).parent().parent().parent().find('ul li a.clicked').hasClass('clicked')) {
							if ($('#tlm' + key).hasClass('clicked')) $('#tlm' + key).removeClass('clicked');
							return;
						}
					...
					if (yearsCont[key] == null || yearsCont[key] == true) {
						$("#tlm" + key).css("color", "");
